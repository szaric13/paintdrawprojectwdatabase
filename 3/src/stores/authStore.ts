...
export const useAuthStore = defineStore('auth', () => {
  ...
  const getPictureById = async (pictureId: string) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/pictures/${pictureId}`);
  ...
  const updatePicture = async (pictureId: string, updates: Partial<{ name: string; picture_data: string[][] }>) => {
    try {
      await axios.patch(
    ...
    if (err instanceof AxiosError && err.response) {
      ...
      switch (errorCode) {
        ...
        case 'INVALID_DATA':
          ...
          break;
        case 'NOT_YOURS':
          alert('You are not allowed to modify this entity.');
          break;
        case 'BAD_PICTURE_DATA':
          alert('Invalid picture data format.');
          break;
        default:
          alert('An unknown error occurred.');
          break;
